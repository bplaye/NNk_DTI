...
def process_DB():
    DB = 'AtomizationEnergy'

    list_ID, list_SMILES, list_y, dict_id2smile = [], [], [], {}
    reader = csv.reader(open('data/AtomizationEnergy/gdb1k_with_smiles.sdf.csv'), delimiter=',')
    i = 0
    for row in reader:
        if i > 0:
            smile = row[0]
...
if __name__ == "__main__":
    # process_DB()
    mol_make_CV('AtomizationEnergy', AtomizationEnergy_CV, 'standard')
